def load_page_3(
    ...
    # Choice of sessions to include in the database (FP have a lot of outlier laps that skew the results)
    session_codes = ["Q", "R"]
    ...
    for sess_code in session_codes:
        try:
            # Load the session to retrieve the data.
            sess = fastf1.get_session(year, event_name, sess_code)
            sess.load()

            # Load the laps for the selected driver
            ...
            for _, lap in laps.iterrows():
                try:
                    ...
                    # Compute average speed and throttle values.
                    ...
                    # Compute total braking time during the lap.
                    brake_time = np.nan
                    ...
                    # Select the used compound and lap number
                    ...
                    # Put all the values computed for the lap together
                    all_rows.append({
            ...
            print(f"Error Occured: Skipping {sess_code}: {e}")
            continue

    # Build the Dataframe with the rows collected for each valid lap.
    ...
    # Cancel all the rows that have a "NaN" Lap Time (target of the ML model, must be meaningful).
    df = df[df["LapTime"].notna()].copy()
    df = df.replace([np.inf, -np.inf], np.nan)

    # Dataset preview.
    # Just if verbose = True (standard).
    if verbose: 
        print("\nDataset preview:")
        print(df)
    ...
    # Define the Xs and the y (target). 
    ...
    # Define the model (SVR with rbf kernel).
    model = Pipeline([
    ...
    # Compute predicted labels with test data.
    y_pred = model.predict(X_test)

    # Performance metrics (MAE, MSE, RMSE, R2).
    ...
    # Scatter plot of y_test and predicted labels for visual evaluation of performances. 
    ...
    # plot a line x = y for reference. 
    min_val = min(y_test.min(), y_pred.min())
    max_val = max(y_test.max(), y_pred.max())
    plt.plot([min_val, max_val], [min_val, max_val])

    # Write the metrics as annotations on the plot. 
    metrics_text = (
        f"R²   = {r2:.3f}\n"
        f"MSE  = {mse:.3f} s²\n"
        f"RMSE = {rmse:.3f} s\n"
    ...
    # return the Dataframe and the Model in case it is wanted to use the model for further evaluations. 
    return df, model