...
# ---------------------------------------------------------
# Page 2: 
# Displays a telemetry for a single lap for 1 or 2 drivers, given the sessions from which loads the data and the channels to display. 

def _get_lap_telemetry(session, driver: str, lap_number):
    """
    ...
    elif lap_number == "fastest":
        lap = laps.pick_fastest()

    else: 
        print ("Lap_Number variable not valid.\n" \
        "Lap Number can only be:\n" \
        "- An integer\n" \
        "- The string \"fastest\" to select the fastest lap. ")
...
def load_page_2(
    ...
    telA = _get_lap_telemetry(sessionA, driver_A, lap_A)
    telB = _get_lap_telemetry(sessionB, driver_B, lap_B)
    ...
    fig.suptitle(f"{sessionA.event.EventName} {sessionA.name} Telemetry Comparison", fontsize=16, fontweight="bold")
    fig.text(
        0.5, 0.93,
        f"A: {driver_A} Lap: {lap_A} {sessionA.event.year}   vs     B: {driver_B} Lap {lap_B} {sessionB.event.year}",
        ha="center",
        fontsize=11
    )

    # Create the grid for the plots: one row per channel.
    n = len(channels)
    ...
    for i, ch in enumerate(channels):
        ...
        # Special handling for Brake if it's boolean.
        if ch.lower() == "brake":
        ...
        if ch.lower() == "time":
            ...
            ax.plot(xA, yA, label = f"Delta Time {driver_A} {sessionA.event.year} - {driver_B} {sessionB.event.year}")
            ax.set_ylabel(f"Delta Time [s] \n{driver_A} {sessionA.event.year} - {driver_B} {sessionB.event.year}")
            continue

        # Plot the selected channels.
        ax.plot(xA, yA, label=f"A: {driver_A} {sessionA.event.year}", linewidth=1.6)
        ax.plot(xB, yB, label=f"B: {driver_B} {sessionB.event.year}", linewidth=1.6)

        # Create labels for each channel available.
        if ch.lower() == "speed":
    ...
    # This part adds a vertical cursor shared between all subplots, that follow the mouse cursor.  
    vlines = []
    for ax in axes:
        vl = ax.axvline(np.nan, linewidth=1.2, alpha=0.8, color = "grey")
        vlines.append(vl)
